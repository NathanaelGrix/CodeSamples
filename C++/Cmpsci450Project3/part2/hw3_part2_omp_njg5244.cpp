...
//function that returns a vector list with all the indexes and values that > the threashold
void largerValues(double* ar, double bar, std::vector<int>* index, std::vector<double>* value){
	//int numLarge=0;
	//split between threads
	#pragma omp parallel num_threads(P)
	{
	std::vector<int> localIndex;
	std::vector<double> localValue;
	//split up the for loop
	#pragma omp for //reduction(+:numLarge)
		for(int i = 0; i<N; i++){
			//get anything that is over the bar
			if(ar[i]>bar){
				//numLarge++;
				localIndex.push_back(i);
				localValue.push_back(ar[i]);
			}
		}
		#pragma omp critical
		{	//combine everything in a safe section
			(*index).insert((*index).end(),localIndex.begin(),localIndex.end());
			(*value).insert((*value).end(),localValue.begin(),localValue.end());
		}
	}
	std::cout<<"num of values > "<<T<<" = "<<(*value).size()<<std::endl;
	//std::cout<<"num of values > "<<bar<<" = "<<numLarge<<std::endl;
}
...
int main(int argc, char** argv){
	...
	//get the values over a certain size
	largerValues(arrayi, T, &ind, &val);
	std::cout<<"num of values > "<<T<<" = "<<val.size()<<std::endl;
